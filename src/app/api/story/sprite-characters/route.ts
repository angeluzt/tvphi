...
const cuerpo=z.object({personajeId:z.string().cuid().optional(),animacionId:z.string().cuid().optional(),nombrePersonaje:z.string().trim().min(1).max(60),
 ...
 anclaje:z.enum(["centro","pies"]),croma:z.string().regex(/^#[0-9a-f]{6}$/i),columnas:z.number().int().min(1).max(24),filas:z.number().int().min(1).max(24),
 anchoHoja:z.number().int().min(1).max(8192),altoHoja:z.number().int().min(1).max(8192),ancho:z.number().int().min(1).max(4096),alto:z.number().int().min(1).max(4096),
 celdas:z.array(celda).min(1).max(24),hojaOriginal:z.string().min(100).max(9_000_000),hojaTrabajo:z.string().min(100).max(9_000_000),
...
export async function POST(req:Request){const u=await getCurrentUser();if(!u)return NextResponse.json({error:"No autorizado"},{status:401});const userId=u.id;
 ...
 if(d.ancho*d.fotogramas>16384)return NextResponse.json({error:"La tira sería demasiado ancha."},{status:400});
 const original=png(d.hojaOriginal,MAX_HOJA),trabajo=png(d.hojaTrabajo,MAX_HOJA),tira=png(d.tira,MAX_TIRA),ref=d.referencia?png(d.referencia,MAX_REF):null;
 if(!original||!trabajo||!tira)return NextResponse.json({error:"Los PNG no son válidos o pesan demasiado."},{status:413});const work=trabajo.equals(original)?null:trabajo;
 const propios=await prisma.spriteCharacter.findMany({where:{userId},select:{id:true,bytesReferencia:true,animaciones:{select:{id:true,bytesOriginal:true,bytesTrabajo:true,bytesTira:true}}}});
 const total=propios.reduce((s,x)=>s+x.bytesReferencia+x.animaciones.reduce((a,y)=>a+y.bytesOriginal+y.bytesTrabajo+y.bytesTira,0),0);
 const atl=await prisma.spriteAtlas.aggregate({where:{userId},_sum:{bytes:true}});const prev=d.animacionId?propios.flatMap(x=>x.animaciones).find(x=>x.id===d.animacionId):null;
 const old=prev?prev.bytesOriginal+prev.bytesTrabajo+prev.bytesTira:0;const nuevo=original.length+(work?.length??0)+tira.length+(!d.personajeId&&!d.animacionId?(ref?.length??0):0);
 if(total+(atl._sum.bytes??0)-old+nuevo>MAX_TOTAL)return NextResponse.json({error:"Tu biblioteca alcanzó 120 MB."},{status:409});
 const data={nombre:d.nombre,que:d.que,fotogramas:d.fotogramas,fps:d.fps,vista:d.vista,direccion:d.direccion,accion:d.accion,anclaje:d.anclaje,croma:d.croma,
 columnas:d.columnas,filas:d.filas,anchoHoja:d.anchoHoja,altoHoja:d.altoHoja,ancho:d.ancho,alto:d.alto,celdas:d.celdas,hojaOriginal:original,hojaTrabajo:work,tira,
 bytesOriginal:original.length,bytesTrabajo:work?.length??0,bytesTira:tira.length};
 const pack=async(id:string)=>{try{return await archivarAnimacionEnAtlas(userId,id);}catch(e){console.error("Se conserva la tira porque falló el atlas",e);return false;}};
 if(d.animacionId){const e=await prisma.spriteAnimation.findFirst({where:{id:d.animacionId,character:{userId}},select:{id:true,characterId:true,hojaOriginal:true,bytesOriginal:true}});
 if(!e)return NextResponse.json({error:"Animación no encontrada."},{status:404});const a=await prisma.spriteAnimation.update({where:{id:e.id},data:{...data,hojaOriginal:e.hojaOriginal,bytesOriginal:e.bytesOriginal}});
 await prisma.spriteCharacter.update({where:{id:e.characterId},data:{nombre:d.nombrePersonaje,descripcion:d.descripcionPersonaje}});return NextResponse.json({ok:true,personajeId:e.characterId,animacionId:a.id,actualizada:true,enAtlas:await pack(a.id)});}
 if(d.personajeId){const c=await prisma.spriteCharacter.findFirst({where:{id:d.personajeId,userId},include:{_count:{select:{animaciones:true}}}});if(!c)return NextResponse.json({error:"Personaje no encontrado."},{status:404});
