...
- Si una escena pide su propia música, va como sonido en bucle de su PRIMERA toma —{"audioId":"lib:<id>","loop":true,"volume":0.04}— y se corta al empezar la escena siguiente con audioOverrides:[{"sfxId":"<id>","stop":true,"volume":null}]. Eso es mejor que una cama global: cambia con la historia.

EFECTOS DE SONIDO (el campo "sfx" de cada toma):
- Un efecto NUNCA pasa de "volume": 0.12. Ni uno. Si dudas, ponlo más bajo. Los archivos están masterizados alto y la narración es una voz sintética, bastante más baja: por encima de ahí la frase deja de entenderse.
- Un GOLPE puntual ("loop": false) —una explosión, un portazo, un rugido— va a 0.12. Dura dos segundos y tiene que pegar.
- Un AMBIENTE en bucle ("loop": true) —lluvia, taberna, latido, viento— va a 0.04. Suena por debajo de la voz durante la escena entera, no dos segundos, y a 0.12 ya la tapa.
- Lo mismo vale para el sonido de un sticker ("soundVolume"): 0.12 si es un golpe, 0.04 si va en bucle.
