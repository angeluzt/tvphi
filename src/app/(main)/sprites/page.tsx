...
export default async function SpritesPage(){const u=await getCurrentUser();if(!u)redirect("/auth/login");return <div className="mx-auto max-w-5xl space-y-4">
 ...
 <div className="card p-4"><div className="flex gap-2"><Bird className="h-5 w-5 text-accent"/><div><h1 className="text-lg font-bold">Taller de personajes animados</h1>
 <p className="mt-1 text-xs text-muted">Cada personaje conserva su referencia y varias animaciones editables; los cuadros finales se compactan en atlas.</p></div></div></div>
