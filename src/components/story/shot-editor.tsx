...
import {
  ...
  type TransitionKind, type OverlayTransition,
} from "@/lib/story/model";

// Panel de una sub-escena (toma): encuadre, duración, transición, diálogos
// narrados, efectos de sonido y stickers.
export function ShotEditor({
  ...
  return (
    <div className={`rounded-xl border bg-surface-2/40 p-3 ${expanded ? "border-brand/60" : "border-border"}`}>
        ...
        <button onClick={onToggle} className="flex min-w-0 flex-1 items-center gap-2 text-left">
          ...
          {!expanded && (
            <span className="truncate text-xs text-muted">
              · {TRANS_LABEL[shot.transition]} {shot.transitionDur.toFixed(1)}s
      ...
      {/* Encuadre */}
      <div className="mt-3">
        <FramingEditor shot={shot} imageId={imageId} imgW={imgW} imgH={imgH} onChange={onChange} />
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <label className="space-y-0.5 text-xs">
          <span className="text-muted">Velocidad (s)</span>
          <input
        ...
        <div className="mt-2 space-y-1">
          {shot.overlays.map((o) => (
            <div
              ...
              className={`flex cursor-pointer flex-wrap items-center gap-2 rounded-lg border px-2 py-1 text-xs ${selectedOverlay === o.id ? "border-accent bg-accent/10" : "border-border hover:bg-surface-2"}`}
            >
          ...
          {!!shot.overlays.length && (
            <p className="text-[11px] text-muted">
              Selecciona uno y arrástralo sobre la previsualización para moverlo o cambiarle el tamaño.
            </p>
          )}
        </div>
      </div>
...
const TRANS_LABEL: Record<TransitionKind, string> = {
  cut: "Corte",
  fade: "Fundido",
  slide: "Deslizar",
};
...
export function newOverlay(imageId: string): PngOverlay {
  return { id: nanoid(6), imageId, x: 0.35, y: 0.35, w: 0.3, h: 0.3, transition: "inherit" };
}
